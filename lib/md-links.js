...
const validateLink = (mdLinks) => {
  ...
  for (let index = 0; index < mdLinks.length; index++) {
    ...
    if (element.href.includes('https')) {
      validatePromises.push(validateLinkHttps(element));
    } else {
      validatePromises.push(validateLinkHttp(element));
    }
  }
  return Promise.all(validatePromises);
};

const validateLinkHttps = (element) => {
  return new Promise((resolve, reject) => {
    const options = { timeout: 12000 };
    https.get(element.href, options, (res) => {
...
const validateLinkHttp = (element) => {
  const options = { timeout: 12000 };
  http.get(element.href, options, (res) => {
    element.status = res.statusCode;
    if (element.status === 200) {
      element.ok = 'ok';
    } else if (element.status === 400) {
